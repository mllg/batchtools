...
mergeRegistries = function(source, target = getDefaultRegistry()) {
  ...
  if (npath(source$file.dir) == npath(target$file.dir))
  ...
  file.copy(
    from = source$uri$results(status),
    to = target$uri$results(status)
  )

  info("Copying logs ...")
  file.copy(
    from = source$uri$logs(status),
    to = target$uri$logs(status)
  )

  ext.dirs = chintersect(list.files(source$uri$path[["external"]]), as.character(status$job.id))
  if (length(ext.dirs) > 0L) {
    info("Copying external directories ...")
    target.dirs = target$uri$external(ext.dirs)
    lapply(target.dirs[!dir.exists(target.dirs)], dir.create)
    file.copy(
      from = source$uri$external(ext.dirs),
      to = rep.int(target$uri$path[["external"]], length(ext.dirs)),
      recursive = TRUE
    )
  }
