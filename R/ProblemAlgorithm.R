...
addProblem = function(name, data = NULL, fun = NULL, seed = NULL, reg = getDefaultRegistry()) {
  ...
  writeRDS(prob, file = file.path(reg$file.dir, "problems", sprintf("%s.rds", digest(name))))
...
removeProblem = function(name, reg = getDefaultRegistry()) {
  ...
  file.remove(file.path(reg$file.dir, "problems", sprintf("%s.rds", digest(name))))
...
addAlgorithm = function(name, fun = NULL, reg = getDefaultRegistry())  {
  ...
  writeRDS(algo, file = file.path(reg$file.dir, "algorithms", sprintf("%s.rds", digest(name))))
...
removeAlgorithm = function(name, reg = getDefaultRegistry()) {
  ...
  file.remove(file.path(reg$file.dir, "algorithms", sprintf("%s.rds", digest(name))))
  reg$defs = reg$defs[!def.ids]
  reg$status = reg$status[!job.ids]
