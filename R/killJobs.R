...
killJobs = function(ids = NULL, reg = getDefaultRegistry()) {
  ...
  cols = c("submitted", "started", "done", "error", "memory", "resource.id", "batch.id", "array.id", "job.hash")
  reg$status[tab[tab$killed], (cols) := list(NA_real_, NA_real_, NA_real_, NA_character_, NA_real_, NA_integer_, NA_character_, NA_integer_, NA_character_)]
