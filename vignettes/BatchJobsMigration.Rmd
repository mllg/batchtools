...
* Nodes do not have to access the registry. [submitJobs()](https://mllg.github.io/batchtools/submitJobs.html) stores an object of type [JobDescription](https://mllg.github.io/batchtools/JobDescription.html) on the file system.
  ...
  Each job stores its status updates and results on the file system and [syncRegistry()](https://mllg.github.io/batchtools/syncRegistry.html) can be used on the master to update the registry.
* Chunks can be parallelized using `parallel` (linux/mac os) or `snow` (windows) by setting the resource `chunk.ncpus`.
