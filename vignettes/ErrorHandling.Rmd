...
Therefore, before you submit anything you should use [`testJob()`](https://mllg.github.io/batchtools/testJob) to catch errors that are easy to spot because they are raised in many or all jobs.
...
```
When you have already submitted the jobs and suspect that something is going wrong, the first thing to do is to run [`getStatus()`](https://mllg.github.io/batchtools/getStatus) to display a summary of the current state of the system.
```{r}
submitJobs()
getStatus()
```
The status message shows that two of the jobs could not be executed successfully.
To get the IDs of all jobs that failed due to an error we can use [`findErrors()`](https://mllg.github.io/batchtools/findJobs) and to retrieve the actual error message, we can use [`getErrorMessages()`](https://mllg.github.io/batchtools/getErrorMessages).
```{r}
findErrors()
getErrorMessages()
```
If we want to peek into the R log file of a job to see more context for the error we can use [`showLog()`](https://mllg.github.io/batchtools/showLog) which opens a pager or use [`getLog()`](https://mllg.github.io/batchtools/showLog) to get the log as character vector:
```{r}
