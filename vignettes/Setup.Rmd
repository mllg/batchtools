...
```{r, include = FALSE}
library(batchtools)
fd = file.path(dirname(tempdir()), "batchtools", "Setup")
...
All variables in a [JobCollection](https://mllg.github.io/batchtools/JobCollection.html) can be used inside the template.
```{r, message = FALSE}
batchMap(identity, 1:2)
jc = makeJobCollection(1:2)
print(ls.str(jc))
```
If the flexibility which comes with templating is not sufficient for your system, you can still construct the cluster functions yourself using the provided [constructor](https://mllg.github.io/batchtools/makeClusterFunctions.html).
