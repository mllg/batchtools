...
```{r, message = FALSE}
library(batchtools)
# reg = makeExperimentRegistry(file.dir = "iris_example")
...
The function must have the arguments `job`, `data` and `problem`.
...
The algorithm can return any R object which will automatically be stored the file system for later retrieval.
Firstly, we create a classification tree learner:
```{r}
tree.wrapper = function(data, job, problem, minsplit, cp, ...) {
  library("rpart")
  mod = rpart(Species ~ ., data = data[problem$train, ], control = list(minsplit = minsplit, cp = cp), ...)
  pred = predict(mod, newdata = data[problem$test, ], type = "class")
  table(data$Species[problem$test], pred)
}
...
```{r}
forest.wrapper = function(data, job, problem, ...) {
  library("ranger")
  mod = ranger(Species ~ ., data = data[problem$train, ], write.forest = TRUE)
  pred = predict(mod, data = data[problem$test, ])
  table(data$Species[problem$test], pred$predictions)
}
...
Each experiment requires a problem design and a algorithm design which must be passed as named lists to `addExperiment()`.
...
# algorithm design: try combinations of minsplit and cp exhaustively,
# try different number of trees for the forest
ades = list(
  tree = expand.grid(minsplit = c(5, 10, 20), cp = c(0.01, 0.1)),
...
To illustrate `findExperiments()`, we will select two experiments, one with a decision tree and the other with a random forest and the parameter `ntree = 1000`.
The selected experiment IDs are then passed to testJob.
```{r}
id1 = findExperiments(algo.name = "tree")[1]
id2 = findExperiments(algo.name = "forest", algo.pars = (ntree == 1000))[1]
testJob(id = id1)
testJob(id = id2)
```

After the jobs are executed, we collect the results with `reduceResultsDataTable()` and directly extract the mean misclassification error:
```{r}
submitJobs()
...
```{r}
tab[ratio == 0.67, list(mmce = mean(mce)), by = c("algorithm", "minsplit", "cp", "ntree")]
```
