...
We use an [`ExperimentRegistry`](https://mllg.github.io/batchtools/ExperimentRegistry) where the job definition is split into creating problems and algorithms.
...
Therefore the function [`addProblem()`](https://mllg.github.io/batchtools/ProblemAlgorithm) accepts static parts in its `data` argument, which is passed to the argument `fun` which generates a (possibly stochastic) problem instance.
...
The argument `job` is an object of type [`Job`](https://mllg.github.io/batchtools/makeJob) which holds additional information about the job.
...
```
[`addProblem()`](https://mllg.github.io/batchtools/ProblemAlgorithm) files the problem to the file system and the problem gets recorded in the registry.
```{r}
...
When using [`addAlgorithm()`](https://mllg.github.io/batchtools/ProblemAlgorithm), an identifier as well as the algorithm to apply to are required arguments.
...
[`addExperiments()`](https://mllg.github.io/batchtools/addExperiments) is used to parametrize the jobs and thereby define computational jobs.
To do so, you have to pass named lists of parameters to [`addExperiments()`](https://mllg.github.io/batchtools/addExperiments).
...
When the problem design and the algorithm design are combined in [`addExperiments()`](https://mllg.github.io/batchtools/addExperiments), each combination of the parameter sets of the two designs defines a distinct job.
...
The function [`summarizeExperiments()`](https://mllg.github.io/batchtools/summarizeExperiments) returns a table which gives a quick overview over all defined experiments.
...
For all these use cases, [`findExperiments()`](https://mllg.github.io/batchtools/findJobs) can be employed to conveniently select a particular subset of jobs.
...
To illustrate [`findExperiments()`](https://mllg.github.io/batchtools/findJobs), we will select two experiments, one with a support vector machine and the other with a random forest and the parameter `ntree = 1000`.
...
To submit the jobs, we call [`submitJobs()`](https://mllg.github.io/batchtools/submitJobs) and wait for all jobs to terminate using [`waitForJobs()`](https://mllg.github.io/batchtools/waitForJobs).
```{r}
submitJobs()
waitForJobs()
```
After jobs are finished, the results can be collected with [`reduceResultsDataTable()`](https://mllg.github.io/batchtools/reduceResultsList) where we directly extract the mean misclassification error:
```{r}
results = reduceResultsDataTable(fun = function(res) (list(mce = (sum(res) - sum(diag(res))) / sum(res))))
head(results)
```

Next, we merge the results table with the table of job parameters using one of the [join helpers](https://mllg.github.io/batchtools/JoinTables) provided by `batchtools` (here, we use an inner join):
```{r}
