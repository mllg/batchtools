...
We use an [`ExperimentRegistry`](https://mllg.github.io/batchtools/reference/ExperimentRegistry) where the job definition is split into creating problems and algorithms.
...
Therefore the function [`addProblem()`](https://mllg.github.io/batchtools/reference/ProblemAlgorithm) accepts static parts in its `data` argument, which is passed to the argument `fun` which generates a (possibly stochastic) problem instance.
...
The argument `job` is an object of type [`Job`](https://mllg.github.io/batchtools/reference/makeJob) which holds additional information about the job.
...
```
[`addProblem()`](https://mllg.github.io/batchtools/reference/ProblemAlgorithm) files the problem to the file system and the problem gets recorded in the registry.
```{r}
...
When using [`addAlgorithm()`](https://mllg.github.io/batchtools/reference/ProblemAlgorithm), an identifier as well as the algorithm to apply to are required arguments.
