...
There are two different types of registry objects: First, a regular [`Registry`](https://mllg.github.io/batchtools/reference/Registry) which we will use in this example.
Second, an [`ExperimentRegistry`](https://mllg.github.io/batchtools/reference/ExperimentRegistry) which provides an alternative way to define computational jobs and thereby is tailored for a broad range of large scale computer experiments (see one of the other vignettes for an example).
...
To get the current default registry, [`getDefaultRegistry`](https://mllg.github.io/batchtools/reference/Registry) can be used.
To switch to another registry, use [`setDefaultRegistry()`](https://mllg.github.io/batchtools/reference/Registry).
