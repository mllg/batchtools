...
There are two different types of registry objects: First, a regular [`Registry`](https://mllg.github.io/batchtools/Registry) which we will use in this example.
Second, an [`ExperimentRegistry`](https://mllg.github.io/batchtools/ExperimentRegistry) which provides an alternative way to define computational jobs and thereby is tailored for a broad range of large scale computer experiments (see one of the other vignettes for an example).
...
To get the current default registry, [`getDefaultRegistry`](https://mllg.github.io/batchtools/Registry) can be used.
To switch to another registry, use [`setDefaultRegistry()`](https://mllg.github.io/batchtools/Registry).
...
We use [`batchMap()`](https://mllg.github.io/batchtools/batchMap) to define the jobs (note that this does not yet start the calculation):
```{r}
...
An overview over the jobs and their IDs can be retrieved with [`getJobTable()`](https://mllg.github.io/batchtools/getJobTable) which returns a data frame with all relevant information:
```{r}
names(getJobTable())
```
Note that a unique job ID is assigned to each job.
These IDs can be used to restrict operations to subsets of jobs.
To actually start the calculation, call [`submitJobs()`](https://mllg.github.io/batchtools/submitJobs).
...
The progress of the submitted jobs can be checked with [`getStatus()`](https://mllg.github.io/batchtools/getStatus).
```{r}
getStatus()
```
...
This can be done in a simple fashion by using either [`loadResult()`](https://mllg.github.io/batchtools/loadResult), which returns a single result exactly in the form it was calculated during mapping, or by using [`reduceResults()`](https://mllg.github.io/batchtools/reduceResults), which is a version of `Reduce()` from the base package for registry objects.
```{r}
mean(sapply(1:10, loadResult))
mean(reduceResults(function(x, y) c(x, y)))
```

If you are absolutely sure that your function works, you can take a shortcut and use *batchtools* in an `lapply` fashion using [`btlapply()`](https://mllg.github.io/batchtools/btlapply).
This function creates a temporary registry (but you may also pass one yourself), calls [`batchMap()`](https://mllg.github.io/batchtools/reduceResultsList), wait for the jobs to terminate with [`waitForJobs()`](https://mllg.github.io/batchtools/waitForJobs) and then uses [`reduceResultsList()`](https://mllg.github.io/batchtools/reduceResultsList) to return the results.
