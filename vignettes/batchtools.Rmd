...
library(batchtools)
options(batchtools.progress = FALSE, datatable.print.class = TRUE)
```
...
To unnest the lists in columns `prob.pars`, `algo.pars` and `results`, [`flatten()`]((https://mllg.github.io/batchtools/reference/flatten) can be used:
```{r}
tab = flatten(tab)
head(tab)
```
...
1. Make sure you have all packages installed. Package versions can be synchronized with [`checkpoint`](https://cran.r-project.org/package=checkpoint) or [`packrat`](https://cran.r-project.org/package=packrat).
2. Test jobs on the remote system before submitting to ensure that paths are resolved correctly.
...
6. Never access the `file.dir` with multiple sessions simultaneously.
   Also never mount the file dir to work with it in a local R session.
   This may lead to inconsistencies and missing results.
