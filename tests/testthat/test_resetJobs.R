...
test_that("resetJobs", {
  ...
  expect_true(file.exists(getLogFiles(reg$file.dir, reg$status[.(3), job.hash])))
  ...
  expect_true(file.exists(getLogFiles(reg$file.dir, reg$status[.(3L), job.hash])))
  ...
  expect_false(file.exists(getLogFiles(reg$file.dir, reg$status[.(3L), job.hash])))
  expect_false(file.exists(getResultFiles(reg$file.dir, 3)))
