...
test_that("resetJobs", {
  ...
  expect_true(file.exists(reg$uri$logs(reg$status[.(3)])))
  expect_false(identical(reg$status$submitted, before$status$submitted))
  expect_true(file.exists(reg$uri$results(1)))
  expect_equal(dir.exists(reg$uri$external(1:3)), c(TRUE, FALSE, TRUE))

  resetJobs(1, reg = reg)
  expect_true(all.equal(before$status[1], reg$status[1]))
  expect_false(file.exists(reg$uri$results(1)))
  expect_true(file.exists(reg$uri$results(3)))
  expect_true(file.exists(reg$uri$logs(reg$status[.(3L)])))
  expect_equal(dir.exists(reg$uri$external(1:3)), c(FALSE, FALSE, TRUE))
  expect_false(file.exists(reg$uri$results(1)))
  expect_true(file.exists(reg$uri$results(3)))
  ...
  expect_false(file.exists(reg$uri$logs(reg$status[.(3L)])))
  expect_false(file.exists(reg$uri$results(3)))
  expect_equal(dir.exists(reg$uri$external(1:3)), c(FALSE, FALSE, FALSE))
})
