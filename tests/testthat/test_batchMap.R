...
test_that("batchMap", {
  ...
  expect_equal(readRDS(reg$paths$file[["user.function"]]), fun)
  expect_equal(readRDS(reg$paths$file[["more.args"]]), list(x = 1))
  ...
  expect_equal(readRDS(reg$paths$file[["more.args"]]), list(j = 1))
  ...
  expect_equivalent(getJobPars(reg = reg)[, c("a", "b")], cj)
})
